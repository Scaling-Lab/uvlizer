...
! function(t, e) {
    ...
    return function(t) {
        ...
        var h = function() {
            ...
            return t = GoCart, r = [{
                ...
                value: function(t) {
                    var e = this; var subtotalold = 0;
                    this.clearCartDrawer(), t.items.forEach((function(t, r) {
                        var a = t.variant_title;
                        var el_compareprice = (document.querySelector('[v_id="'+t.variant_id+'"]').getAttribute('v_cp')||'').substr(1);
                        var comparepricing="";
                        if (el_compareprice!=''){
                          el_compareprice = parseFloat(el_compareprice)*100;
                          comparepricing = parseInt(el_compareprice) * parseInt(t.quantity);
                          subtotalold += comparepricing;
                          comparepricing = formatMoney(comparepricing, e.moneyFormat)
                        }
                      
                        null === a && (a = "");
                        var o = '\n        <div class="go-cart-item__single "  data-line="'.concat(Number(r + 1), '">\n            <div class="go-cart-item__info-wrapper">\n                <div class="go-cart-item__image" id="p-').concat(t.product_id, '" style="background-image: url(').concat(t.image, ');"></div>\n                <div class="go-cart-item__info">\n                    <a href="').concat(t.url, '" class="go-cart-item__title">').concat(t.product_title, '</a>\n                    <div class="go-cart-item__variant">').concat(a, '</div>\n                    <div class="go-cart-item__quantity">\n                        <span class="go-cart-item__quantity-label">').concat(e.labelQuantity, ' </span>\n                        <span class="go-cart-item__quantity-button js-go-cart-quantity-minus">-</span>\n                        <input class="go-cart-item__quantity-number js-go-cart-quantity" type="number" value="').concat(t.quantity, '" disabled>\n                        <span class="go-cart-item__quantity-button js-go-cart-quantity-plus">+</span>\n                    </div>\n                </div>\n            </div>\n            <div class="go-cart-item__price ssss"><span id="c-').concat(t.product_id, '"></span>').concat(formatMoney(t.line_price, e.moneyFormat),  '<span class="old-price">').concat(comparepricing, '</span></div>\n            <a class="go-cart-item__remove ').concat(e.removeFromCartNoDot, '">').concat(e.labelRemove, "</a>\n        </div>\n      ");
                        e.cartDrawerContent.innerHTML += o
                    })), this.cartDrawerSubTotal.innerHTML = (formatMoney(t.total_price, this.moneyFormat)+'<span class="saved-price icon-wrapper">You saved '+formatMoney((subtotalold-t.total_price), this.moneyFormat)+'</span> '), this.cartDrawerSubTotal.parentNode.classList.remove("is-invisible"), document.querySelectorAll(this.removeFromCart).forEach((function(t) {
 ...
 $(document).on('click','.addcart',function(){
            ...
            var id= $(this).attr('v_id');
   			 var compare_at_price = $(this).attr('v_cp');
            $.ajax({
                type: 'POST',
                url: '/cart/add.js',
                dataType: 'json',
              data: {id:id,quantity:quantity, compare_at_price:compare_at_price},
                success: function(){
